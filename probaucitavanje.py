...
def generate_human_model(filename="simple_human.xml"):
    ...
    k = 1  # koeficijent skaliranja - menjacemo ga na osnovu visine korisnika
    maleHeight=172
    femaleHeight=156
    rgbaIn = "0.8 0.6 0.2 1"
    pol = input("Unesite pol (musko / zensko): ").strip().lower() # Unos pola korisnika
    if pol == "musko":
        sheet_name = 0
        k=float(k/maleHeight)
        rgbaIn = "0.2 0.4 0.8 1"
    elif pol == "zensko":
        sheet_name = 1
        k=float(k/femaleHeight)
        rgbaIn = "0.9 0.4 0.6 1"
    else:
        print("Greška: morate uneti 'musko' ili 'zensko'")
        exit()

    df = pd.read_excel("AnthropometricTableComplete.xlsx", sheet_name=sheet_name)
    visina = float(input("Unesite visinu ispitanika u cm: "))
    k=k*float(visina)
    masskg = input("Unesite masu ispitanika u kg: ")

    # Prva kolona su imena segmenata, druga kolona su dužine
    segments = df.iloc[:, 0]
    lengths = df.iloc[:, 1] * k   # skaliranje vrednosti
    masspercent = df.iloc[:, 2]
    massCenterx = df.iloc[:, 3]
    massCentery = df.iloc[:, 4]
    massCenterz = df.iloc[:, 5]

    # Racunje mase segmenata na osnovu procenta mase i ukupne mase korisnika
    mass = (masspercent / 100) * float(masskg)

    # Napravi dict
    lengths_dict = dict(zip(segments, lengths))
    mass_dict = dict(zip(segments, mass))
    massCenterx_dict = dict(zip(segments, massCenterx))
    massCentery_dict = dict(zip(segments, massCentery))
    massCenterz_dict = dict(zip(segments, massCenterz))
    
    # Novi dict: lokalna pozicija centra mase
    posMCx_dict = {}
    for i in lengths_dict:
        duzinax = lengths_dict[i]
        privremenapx = float(massCenterx_dict[i]) /100
    ...
    for i in lengths_dict:
        ...
        privremenapy = float(massCentery_dict[i]) /100
        # procenat dužine -> realna dužina -> lokalni pomak
    ...
    for i in lengths_dict:
        ...
        privremenapz = float(massCenterz_dict[i]) /100
        # procenat dužine -> realna dužina -> lokalni pomak
    ...
    ET.SubElement(thorax, "geom", type="capsule", size=f"0.1 {lengths_dict['Thorax']/2}", euler="90 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(head, "geom", type="sphere", size=f"{lengths_dict['Head with Neck']/2}", euler="90 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(abdomen, "geom", type="capsule", size=f"0.1 {lengths_dict['Abdomen']/2}", euler="90 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(pelvis, "geom", type="capsule", size=f"0.1 {lengths_dict['Pelvis']/2}", euler="90 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(Left_thigh, "geom", type="capsule", size=f"0.04 {lengths_dict['Thigh']/2}", euler="0 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(Right_thigh, "geom", type="capsule", size=f"0.04 {lengths_dict['Thigh']/2}", euler="0 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(Left_shank, "geom", type="capsule", size=f"0.035 {lengths_dict['Shank']/2}", euler="0 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(Right_shank, "geom", type="capsule", size=f"0.035 {lengths_dict['Shank']/2}", euler="0 0 0", rgba=rgbaIn)
    ...
    ET.SubElement(Left_foot, "geom", type="capsule", size=f"0.05 {lengths_dict['Foot']/2}", euler="90 90 0", rgba=rgbaIn)
    ...
    ET.SubElement(Right_foot, "geom", type="capsule", size=f"0.05 {lengths_dict['Foot']/2}", euler="90 90 0", rgba=rgbaIn)
    ...
    ET.SubElement(Left_upper_arm, "geom", type="capsule", size=f"0.035 {lengths_dict['Upper Arm']/2}", rgba=rgbaIn)
    ...
    ET.SubElement(Right_upper_arm, "geom", type="capsule", size=f"0.035 {lengths_dict['Upper Arm']/2}", rgba=rgbaIn)
    ...
    ET.SubElement(Left_forearm, "geom", type="capsule", size=f"0.03 {lengths_dict['Forearm']/2}", rgba=rgbaIn)
    ...
    ET.SubElement(Right_forearm, "geom", type="capsule", size=f"0.03 {lengths_dict['Forearm']/2}", rgba=rgbaIn)
    ...
    ET.SubElement(Left_hand, "geom", type="capsule", size=f"0.025 {lengths_dict['Hand']/2}", rgba=rgbaIn)
    ...
    ET.SubElement(Right_hand, "geom", type="capsule", size=f"0.025 {lengths_dict['Hand']/2}", rgba=rgbaIn)
